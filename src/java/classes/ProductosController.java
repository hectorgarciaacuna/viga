...
import session.ProductosFacade;
...
public class ProductosController implements Serializable {
    ...
    @EJB
    private session.ProductosFacade ejbFacade;
    ...
    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/productos/Bundle").getString("ProductosCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/productos/Bundle").getString("ProductosUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/productos/Bundle").getString("ProductosDeleted"));
        if (!JsfUtil.isValidationFailed()) {
        ...
        if (selected != null) {
            ...
            try {
                ...
                if (msg.length() > 0) {
                    ...
                    JsfUtil.addErrorMessage(ex, ResourceBundle.getBundle("/productos/Bundle").getString("PersistenceErrorOccured"));
                }
            } catch (Exception ex) {
